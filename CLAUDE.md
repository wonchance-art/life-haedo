...
- **지도**: 타일 = `tile.openstreetmap.org`(폴백 `tile.openstreetmap.de`). **carto light_all은 쓰지 말 것** — 키 없이 쓰면 워터마크 타일을 200으로 주고 tileerror가 안 나 폴백도 안 걸린다. 타일 레이어에 `noWrap`뿐 아니라 **`bounds:WORLD_BOUNDS`가 반드시 필요**하다(EPSG3857의 wrapLng 때문에 noWrap만으로는 범위 밖 x를 계속 요청해 400이 난다). 지도 `minZoom:1` + `maxBounds`(세계) + `maxBoundsViscosity:1`.
