...
**배치는 고르지 않는다.** 한 칸(한 주/한 달/하루)과 한 열(한 해/반년/분기/한 달)을
열이 칸의 정수배인 짝으로만 묶고, 폭에 칸이 가장 많이 들어가는 짝을 고른다.
**높이는 채우려 들지 않는다** — 이게 핵심이다. 연표 비율(2.71:1)을 정사각 칸으로 채우려면
한 열이 9달(주 칸)이나 18.8달(달 칸)이라야 하는데 자연스러운 단위에 없다.
2주 칸이면 맞지만 남은 칸을 셀 수 없다 — 칸은 날·주·달만 쓸 것.

칸 최소 크기는 5.5px → 4 → 3 으로 한 단계씩 양보한다(좁은 창). 실측:

| 폭 | 배치 | 칸 | 띠 높이 |
|---|---|---|---|
| 1375px | 160×26 (반년/주) | 8.3px | 250px |
| 725px | 80×12 (한 해/달) | 8.5px | 136px |
| 370px | 80×12 (한 해/달) | 4.1px | 83px |
